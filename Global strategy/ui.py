...
class UI:
    ...
    def update_sidebar(self,selected_country,player,buy_units,enter_attack,enter_move):
        ...
        if selected_country and selected_country in player.territories:
            self.sidebar.add_button("Buy Units", buy_units)
            self.sidebar.add_button("Attack", enter_attack)
            self.sidebar.add_button("Move Units", enter_move)

    def update_player_sidebar(self,player,enemy):
        if player is None:
            return 
        self.player_sidebar.buttons.clear()
        self.player_sidebar.add_button(f"Territories: {len(player.territories)}", None)
        self.player_sidebar.add_button(f"Total Units: {sum(country.units for country in player.territories)}", None)
        self.player_sidebar.add_button(f"Enemy territories: {len(enemy.territories)}", None)
        self.player_sidebar.add_button(f"Enemy units: {sum(country.units for country in enemy.territories)}", None)
        self.player_sidebar.add_button(f"Enemy income: {enemy.income}", None)
        self.player_sidebar.add_button(f"Enemy currency: {enemy.currency}", None)
    ...
    def draw(self,screen,world,state,selected_country,player,enemy,game_result):
        ...
        if selected_country:
            self.sidebar.draw(screen, selected_country)
        if player:
            self.draw_player_stats(player, screen)
        if self.player_sidebar_visible:
            self.player_sidebar.draw(screen, None)
        if state == "game_over":
